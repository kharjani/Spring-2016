...
LListInt LListInt::operator+(const LListInt& other) const{
	LListInt *newList = new LListInt(*this);
	// you can delete the newList in the file where you're acually using the c = a + b -> delete c
	// if copy constructor works, make a copy of 'this' and use a for loop to add the elements of other, then return the new list
	//newList->size_ = this->size_;
...
void LListInt::remove(int loc)
{
	if ((loc > size_ -1) || (loc < 0)){
		...
		if (loc == 0){ // removing the head
			//if(head_->next == NULL){cout << "NULL HEAD NEXT" << endl;}
			//if(head_->prev == NULL){cout << "NULL HEAD PREV" << endl;}
			head_ = head_->next;
			head_->prev = NULL;
		}
